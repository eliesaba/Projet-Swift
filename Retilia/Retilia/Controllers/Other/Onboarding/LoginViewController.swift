...
class LoginViewController: UIViewController {
    ...
    // Utiliser une constant pre definie pour faciliter l'ecriture du code
    struct Constants {
        static let cornerRadius: CGFloat = 8.0
    }
    
    // l'utilisateur a l'option soit d'utiliser email avec password soit username avec password
    private let usernameEmailField: UITextField = {
        ...
        field.layer.cornerRadius = Constants.cornerRadius // Constants is defined in the struct above
    ...
    private let passwordField: UITextField = {
        let field = UITextField()
        field.isSecureTextEntry = true // Le champ va pas montrer le caractere reel et ainsi il montrera *
    ...
    override func viewDidLayoutSubviews() {
        ...
        // utilisation des variables definie dans le fichier 'Extensions' situe au dossier Ressources
    ...
    @objc private func didTapLoginButton() {
        ...
        guard let usernameEmail = usernameEmailField.text, !usernameEmail.isEmpty,
              // Interdire que le code secret soit de taille inferieur a 8
       ...
       // Pour savoir si l'utilisateur a fait acces avec un username ou un mail
        ...
        AuthManager.shared.loginUser(username: username, email: email, password: password) { success in
            DispatchQueue.main.async {
                if success {
                    // l'utilisateur a connecte
                    self.dismiss(animated: true, completion: nil)
                }
                else {
                    // Erreur et affichage d'une alerte
